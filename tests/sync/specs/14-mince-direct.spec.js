...
const { test, expect, g, raw, outboxOps, signIn, waitSynced, seededDevice, fixture } = require('../lib/helpers');
...
test('65 an increment change an older build queued offline still replays safely after the upgrade', async ({ device, backend }) => {
  ...
  expect((await backend.state(a.owner)).doc.settings).toEqual({ totalContainers: 23, mincePurchaseIncrementKg: 1 });
