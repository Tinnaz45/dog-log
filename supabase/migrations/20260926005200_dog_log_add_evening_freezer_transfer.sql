-- WORK-147: after each successfully processed 18:00 Dinner slot, move up to
-- two remaining Full Containers from Freezer to Fridge. Dinner is processed first.
-- The existing meal_events identity and state-row lock provide exactly-once semantics.
alter table dog_log.meal_events
  add column if not exists freezer_to_fridge_count smallint
  check (freezer_to_fridge_count between 0 and 2);
comment on column dog_log.meal_events.freezer_to_fridge_count is
  'WORK-147: Dinner transfer ledger; NULL = feature not run for this meal, otherwise actual Full Containers moved (0..2).';
